...
- The mark (kraft parcel with a map pin) is drawn once in `scripts/brand-mark.mjs`. `scripts/generate-icons.mjs` renders `public/logo.svg`, `app/icon.svg`, `app/favicon.ico`, the manifest/apple/maskable PNGs and the monochrome `badge-96.png` from it - rerun it, never hand-edit them. `components/Brand.tsx` pairs the mark with the two-tone wordmark; `/social-card` embeds the same SVG. `scripts/render-promo.mjs` draws the 8-second silent promo (`public/promo.mp4`, 1080x1080 H.264 for feed posts) frame by frame from the same module; it needs an ffmpeg with libx264 (`FFMPEG=/path/to/ffmpeg`) and the Lato font, and `--keyframes` writes a contact sheet to eyeball before a full render.
