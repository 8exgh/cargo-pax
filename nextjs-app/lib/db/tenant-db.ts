...
export function insertEvent(tenantId: string, params: InsertEventParams): void {
  ...
  db.prepare(`
    INSERT INTO events (aggregate_id, event_type, event_data, payload_blob, timestamp, version)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(
    params.aggregateId,
    params.eventType,
    params.eventData,
    params.payloadBlob || null,
    params.timestamp,
    params.version
  );
}
...
// Binary payloads ride on the event that produced them instead of a
// separate store: here the parsed body of every email forwarded to an
// account's @cargopax.ca address (email_message_received).
export function getPayloadBlob(
  tenantId: string,
  aggregateId: string,
  eventType: string
): Buffer | null {
  ...
  const result = db.prepare(`
    ...
    LIMIT 1
  `).get(aggregateId, eventType) as { payload_blob: Buffer | null } | undefined;

  return result?.payload_blob || null;
}

export function getPayloadBlobByEventId(tenantId: string, eventId: number): Buffer | null {
  const db = getTenantDb(tenantId);
  const result = db.prepare('SELECT payload_blob FROM events WHERE id = ?')
    .get(eventId) as { payload_blob: Buffer | null } | undefined;
  return result?.payload_blob || null;
}
