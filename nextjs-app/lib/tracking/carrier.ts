...
/* Which carrier a tracking url belongs to, and the tracking number inside
   it. Mirrors background-processor/src/utils/carrier.ts (keep both in
   sync): the app uses it for urls pasted on the dashboard, the processor
   for urls found in forwarded emails. */
...
export const COMPANY_LABELS: Record<DeliveryCompany, string> = {
  ups: 'UPS',
  fedex: 'FedEx',
  usps: 'USPS',
  dhl: 'DHL',
